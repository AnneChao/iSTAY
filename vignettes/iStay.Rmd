...
<li> Chao, A., Colwell, R. K., Shia, J., Thorn, S., Yang, M.-Y., Mitesser, O., et al. (2025) A continuum of information-based temporal stability measures and their decomposition across hierarchical level. *BioRxiv* [doi:10.1101/2025.08.20.671203](https://doi.org/10.1101/2025.08.20.671203)
...
- **iSTAY_Multiple**: computes gamma, alpha, and beta stability, as well as synchrony, for multiple time-series of biomass or other relevant variables. The argument `equal_weights` controls whether all time series are assigned equal weights (`TRUE`) or weighted by their total biomass/abundance (`FALSE`).
...
Run the following code to compute the gamma, alpha and beta stability, as well as synchrony values of q = 0.1 to q = 2 in increments 0.2 for two selected metacommunities ("B1_1" and "B3_2"). We show both equal-weighted and biomass-weighted analyses. Only the first ten rows of each output are displayed.
...
The following code returns the gamma, alpha, and beta stability profiles, as well as synchrony profiles, using equal weights.
...
The following code returns the gamma, alpha, and beta stability profiles, as well as synchrony profiles, using biomass weights.
...
The following code returns the relationships between diversity and gamma, alpha, and beta stability, as well as synchrony, based on 20 metacommunities using equal weights.
...
The following code returns the relationships between diversity and gamma, alpha, and beta stability, as well as synchrony, based on 20 metacommunities using biomass weights.
...
## Example 7: Comparing the gamma, alpha, and beta stability profiles, as well as the synchrony profiles in two selected metapopulations

Run the following code to compute the gamma, alpha and beta stability, as well as synchrony values of q = 0.1 to q = 2 in increments 0.2 for two selected metapopulations ("B1A04_B1_4" and "B4A14_B4_2"). We show both equal-weighted and biomass-weighted analyses. Only the first ten rows of each output are displayed.
...
head(output_two_metapopulations_equal_q, 10)
```
...
The following code returns the gamma, alpha, and beta stability profiles, as well as synchrony profiles for the two selected metapopulations using equal weights.
...
head(output_two_metapopulations_biomass_q, 10)
```
...
The following code returns the gamma, alpha, and beta stability profiles, as well as synchrony profiles for the two selected metapopulations using biomass weights.
...
## Example 8: Assessing the relationships between diversity and gamma, alpha, and beta stability, as well as synchrony, based on 76 metapopulations
...
rownames(output_metapopulations_equal_div) <- NULL
head(cbind(output_metapopulations_equal_div[, 1:2],
           round(output_metapopulations_equal_div[, 3:6], 3),
           output_metapopulations_equal_div[, 7:9]), 10)
```
...
The following code returns the relationships between diversity and gamma, alpha, and beta stability, as well as synchrony, based on 76 metapopulations using equal weights.
...
rownames(output_metapopulations_biomass_div) <- NULL
head(cbind(output_metapopulations_biomass_div[, 1:2],
           round(output_metapopulations_biomass_div[, 3:6], 3),
           output_metapopulations_biomass_div[, 7:9]), 10)
```
...
The following code returns the relationships between diversity and gamma, alpha, and beta stability, as well as synchrony, based on 76 metapopulations using biomass weights.

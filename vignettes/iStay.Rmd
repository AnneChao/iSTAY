...
The `iSTAY` package can be installed either from CRAN or from the GitHub repository at [iSTAY_github](https://github.com/AnneChao/iSTAY). For first-time users, the visualization package (`ggplot2`) should be also installed and loaded.
...
- **`Data_Jena_76_communities`**: The biomass data for all species sown within each plot are used to form 76 community datasets. Because the number of species sown per plot ranges from 1 to 16, the number of populations within a community also ranges from 1 to 16. Each community dataset contains the 21-year biomass time series for all constituent populations.
...
The dataset `Data_Jena_20_metacommunities` can be converted into data for the 76 individual plots (communities), stored as `individual_plots` using the following code. The table below displays the data for the first 10 plots and the first five columns of the resulting dataset.
...
The following code extracts the records for the first community (plot), `B1A01_B1_16`, corresponding to B1A01. This community consists of 16 populations (species). Only the first 10 populations and the first five columns of the resulting dataset are displayed.
...
head(round(communities[[1]][,1:5],2), 10)
```
#### Converting community data to population data
The dataset `Data_Jena_76_communities` can be converted into data for the 462 individual populations, stored as `individual_populations`, using the following code. The table below displays the first 10 populations and the first five columns of the resulting dataset. 
...
The converted 462-population dataset (`individual_populations`) are identical to `Data_Jena_462_populations`. For example, the following code displays the first ten rows and the first five columns of `Data_Jena_462_populations`: 
...
Run the following code to compute stability values for orders q = 0.1 to q = 2.0 in increments 0.2 for two selected plots (`B1_4.B1A04` and `B4_2.B4A14`). The first ten rows of the resulting output are displayed below.
...
Run the following code to compute stability values for orders q = 0.1 to q = 2.0 in increments 0.2 for two selected populations (`Ant.odo` and `Cam.pat`) from Plot B1A06. The first ten rows of the resulting output are displayed below.
...
Run the following code to compute gamma, alpha and beta stability, together with synchrony, for orders q = 0.1 to q = 2.0 in increments 0.2 for the two selected communities (`B1A04_B1_4` and `B4A14_B4_2`). Results are presented for both equal-weighted and biomass-weighted analyses. Only the first 10 rows of each output are displayed.
...
```{r, echo=FALSE}
output__biomass_div <- iSTAY_Multiple(
  data = communities,
  order.q = c(1, 2),
  equal_weights = FALSE,
  Alltime = TRUE
...
Run the following code to compute gamma, alpha, and beta stability, as well as synchrony values for orders q = 0.1 to q = 2.0 in increments 0.2 for two selected metacommunities, `B1_1` and `B3_2`). Both equal-weighted and biomass-weighted analyses are presented. For each analysis, only the first 10 rows of the resulting output are displayed.
